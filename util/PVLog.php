...
class PVLog extends PVStaticObject {
	 ...
	 * 			-'directory' _string_: The directory to place the logged file. Default location is '/tmp/' folder
	 * 			-'file' _string_: The file name to save the log in. Default is a blank string. If left empty, the default
	 * 			will be the priorioty name. If the priority is 'alert', the file will be named alert.log.
	 * 			-'timestamp_format' _string_: The date/time format to save the log file as. Default is 'Y-m-d H:i:s'
	 * 			-'log_format' _string_: The format to save the log in. Values will be replaced with passed in values.
	 ...
	 * 
	 * @param string $priority The priority should be a string. Keep in mind that if no 'file' was set in the config, the priority will be the log file name
	 ...
	 * 			-'timestamp_format' _string_: The date/time format to save the log file as. Default is set in the init
	...
	public static function writeLog($priority, $message, $options = array()) {
		...
		$filtered = self::_applyFilter(get_class(), __FUNCTION__, array('priority' =>  $priority, 'message' => $message, 'options' => $options), array('event' => 'args'));
		...
		$written = PVFileManager::writeFile($options['directory'].$options['file'], $log_message, 'a');
	...
	public static function readLog($priority, $options = array()) {
		...
		$filtered = self::_applyFilter(get_class(), __FUNCTION__, array('priority' =>  $priority, 'options' => $options), array('event' => 'args'));
		$priority = $filtered['priority'];
		$options = $filtered['options'];
		...
		if(is_readable($options['directory'].$options['file']))
			$log = PVFileManager::readFile($options['directory'].$options['file']);
